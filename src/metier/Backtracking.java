...
public class Backtracking
{
    ...
    {
    	Case[][] cellules = game.getCellules();
    	int n = (cellules.length+1)/2;
    	valGrille = new int[n][n];
        contraintesHoriz = game.contraintesHoriz;
        contraintesVert = game.contraintesVert;
        
    	for(int i=0;i <cellules.length;i++)
    	{
    		for(int j=0;j<cellules.length;j++)
    		{
    			if(cellules[i][j].getType()==Case.NUMBER && !cellules[i][j].getText().isEmpty())
        ...
        for(String adj: g.adjacentTo(variable))
        {
            if(!adj.contains("s") && !adj.contains("i")){
                if(config.get(adj).equalsIgnoreCase(value))
                    return false;
            }else if(adj.contains("s")) //si l'adj est sup de x
            {
                String orig = adj.replace("s", "x");
                if(!config.get(orig).equals(""))
                {
                    int vN = Integer.parseInt(value);
                    int sN = Integer.parseInt(config.get(orig));
                    //consistent fails if  s < x
                    if(sN <= vN)
                        return false;
                }
            }else //si l'adj est inf de x
            {
                String orig = adj.replace("i", "x");
                if(!config.get(orig).equals(""))
                {
                    int vN = Integer.parseInt(value);
                    int iN = Integer.parseInt(config.get(orig));
                    //consistent fails if i > x
                    if(iN > vN)
                        return false;
                }
            }
